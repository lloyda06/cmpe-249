...
print("i -> speed up, m -> slow down, j -> left, l -> right, space -> STOP, s -> save, d -> destroy")
...
async def drive(car):
    ...
    while True:
        if car.speed > 0:
            img = car.snap(toFile=False)
            img = cv2.resize(img, size)
            classify.set_input(interpreter, img)
            start = time.perf_counter()
            interpreter.invoke()
            inference_time = time.perf_counter() - start
            steer = classify.get_output(interpreter)
            print('%.1fms steer-> %.1f' % (inference_time * 1000, steer))

            #since changeDirection takes a delta, just
            #calculate that delta from the current steer
            car.changeDirection(steer - car.direction)

        await asyncio.sleep(0.1)


async def main():
    t1 = asyncio.create_task(drive(rc_car))
...
interpreter = classify.setup_model("/home/mendel/mobilenet_v2_1.0_224_quant_edgetpu.tflite")
